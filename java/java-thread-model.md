...
_线程 = 线程控制快 + 进程每个任务 + 进程数据_

起初线程的概念是由 Windows 实现并提出，称为轻量级进程。线程作为进程内部的工作单元，每个进程可以有一个或多个线程，每个线程可以共享同任务中的进程数据。因为线程的创建在进程内部完成而无需向系统申请资源，所以开销项比子进程小很多。因为线程能按照相关的任务逻辑来使用进程的资源结构和进程基本一致。在 CPU 指令的是多个线程交替指令。
...
Java 中 Hotspot 规范是典型的一对一线程模型，JVM 只做了封装和调用的方法。
...
JAVA 的 `start0()` 方法是通过 JVM 调用操作系统的线程 `pthread_create` ,最后是使用了操作系统内核的线程。

尝试在 JDK 搜索 `start0` 相关字样，在 JDK 源码中 `Thread.c` 文件中有这么一段代码：
...
在 `jvm.cpp` 这个文件中使用了这个结构体 `JVM_StartThread`。
...
## JNI(Java native interface) 对 Java 线程的模拟
...
```c
JNIEXPORT void JNICALL Java_MyThread_start1(JNIEnv *, jobject);
```
